...
def hide_data(image_path, identifier, secret_message, hide_in_metadata, hide_in_steganography):
    ...
    # Appliquer la stéganographie
    if hide_in_steganography:
        img = lsb.hide(image_path, secret_message)

    # Sauvegarde de l'image en PNG avec les métadonnées EXIF
    output_path = image_path.replace(".png", "_encoded.png").replace(".jpg", "_encoded.png").replace(".jpeg", "_encoded.png")

    metadata = PngImagePlugin.PngInfo()
    metadata.add_text("EXIF", exif_bytes.decode("latin1", "ignore"))  # Convertir EXIF en texte
    
    img.save(output_path, "PNG", pnginfo=metadata)  # Sauvegarde avec EXIF
    return output_path
...
def extract_metadata(image_path):
    ...
    exif_data = img.info.get("EXIF", "").encode("latin1", "ignore")
